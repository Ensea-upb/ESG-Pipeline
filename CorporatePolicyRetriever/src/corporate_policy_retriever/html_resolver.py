...
class HTMLToPDFLinkResolver:
    ...
    def resolve(
        ...
        if self._response_is_pdf(response=response, content_type=content_type):
            return ResolvedPDFLinks(
                source_url=url,
                status="direct_pdf_response",
                message="La page candidate renvoie deja un PDF ; resolution HTML ignoree.",
                candidates=[
                    SearchCandidate(
                        title=page_candidate.title,
                        url=response.url,
                        snippet=page_candidate.snippet,
                        source_name=page_candidate.source_name,
                    )
                ],
                http_status_code=response.status_code,
                content_type=content_type,
        ...
        if not self._looks_like_html_response(content_type=content_type, response=response):
            return ResolvedPDFLinks(
                source_url=url,
                status="not_html_response",
                message=(
                    "La reponse ne semble pas etre du HTML exploitable "
        ...
        try:
            ...
            try:
                ...
                return ResolvedPDFLinks(
                    ...
                    message=f"Erreur pendant le parsing HTML : {exc}",
                    candidates=[],
                    http_status_code=response.status_code,
                    content_type=content_type,
                )
    ...
    @staticmethod
    def _response_is_pdf(response: requests.Response, content_type: str) -> bool:
        lower_content_type = (content_type or "").lower()
        return (
            "application/pdf" in lower_content_type
            or response.content[:5] == b"%PDF-"
        )

    @staticmethod
    def _looks_like_html_response(
        content_type: str,
        ...
        if lower_content_type:
            if (
                ...
                or "application/xhtml+xml" in lower_content_type
                or "application/xml" in lower_content_type
