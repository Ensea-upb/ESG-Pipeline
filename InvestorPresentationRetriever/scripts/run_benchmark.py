...
def print_summary(records, label):
    print(f"\n{'='*90}\nSYNTHESE DU BENCHMARK INVESTOR PRESENTATION\n{'='*90}")
...
def main():
    ...
    retriever = InvestorPresentationRetriever(root_dir=root_dir)
    records = []
    ...
    print_summary(records, "INVESTOR PRESENTATION")
    registry_dir = root_dir / "registry"; registry_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_label = "INVESTOR PRESENTATION".lower().replace(" ", "_")
    output_path = registry_dir / f"benchmark_{safe_label}_{args.fiscal_year}_{timestamp}.json"
    output_path.write_text(json.dumps({"benchmark_name": f"INVESTOR PRESENTATION_retriever_benchmark",
