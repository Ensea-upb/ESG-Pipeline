...
from .models import Company, DownloadResult, InvestorPresentationRequest, ScoredCandidate
from .scorer import InvestorPresentationScorer
from .search import InvestorPresentationSearch
from .storage import InvestorPresentationStorage
from .utils import extract_years, safe_filename, slugify


class InvestorPresentationRetriever:
    def __init__(self, root_dir="data/dossier_ingestion_0",
        ...
        self.search = InvestorPresentationSearch()
        self.scorer = InvestorPresentationScorer(auto_download_threshold=auto_download_threshold,
                                human_review_threshold=human_review_threshold)
        self.downloader = PDFDownloader()
        self.storage = InvestorPresentationStorage(root_dir=self.root_dir)
        self.html_resolver = HTMLToPDFLinkResolver()

    def download_high_score_candidates(self, company, fiscal_year, min_score=80.0):
        request = InvestorPresentationRequest(company=company, fiscal_year=fiscal_year)
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in self.scorer.rank_candidates(request=request, candidates=resolved.candidates):
                    ...
                    if download_response.status != "download_success":
                        ...
                        storage_rank += 1
                        continue
                    manifest = self.storage.store_candidate_investor_presentation(temp_pdf_path=download_response.output_path,
        ...
        return DownloadResult(status="completed", message="Ingestion des candidats présentations investisseurs terminée.",
    ...
    def _build_temp_pdf_path(self, company, fiscal_year, source_url):
        ...
        filename = safe_filename(source_url.split("/")[-1] or "investor_presentation.pdf")
