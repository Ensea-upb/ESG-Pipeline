...
from .models import Company, DownloadResult, VigilancePlanRequest, ScoredCandidate
from .scorer import VigilancePlanScorer
from .search import VigilancePlanSearch
from .storage import VigilancePlanStorage
from .utils import extract_years, safe_filename, slugify


class VigilancePlanRetriever:
    """
    Moteur central d'ingestion des plans de vigilance et rapports de
    devoir de vigilance.

    Cible :
    - Plan de vigilance (loi française du 27 mars 2017) ;
    - Modern Slavery Statement (UK MSA 2015, Australie) ;
    - Human Rights Due Diligence Report ;
    - Supply Chain Transparency / Responsible Sourcing Report.

    Stocke dans : data/dossier_ingestion_0/vigilance_plans/
    """
    ...
    def __init__(
        ...
        self.search = VigilancePlanSearch()
        self.scorer = VigilancePlanScorer(
            auto_download_threshold=auto_download_threshold,
            human_review_threshold=human_review_threshold,
        )
        self.downloader = PDFDownloader()
        self.storage = VigilancePlanStorage(root_dir=self.root_dir)
    ...
    def download_high_score_candidates(
        ...
        """
        Télécharge tous les candidats plans de vigilance dont le score
        est supérieur ou égal à min_score.

        Règle temporelle : accepte fiscal_year et fiscal_year - 1.
        """
        request = VigilancePlanRequest(company=company, fiscal_year=fiscal_year)
        ...
        for source_search_rank, candidate in enumerate(selected_candidates, start=1):
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in self.scorer.rank_candidates(
                    ...
                    manifest = self.storage.store_candidate_vigilance_plan(
        ...
        return DownloadResult(
            status="completed",
            message="Ingestion des candidats plans de vigilance / devoir de vigilance terminée.",
    ...
    def _build_temp_pdf_path(self, company: Company, fiscal_year: int, source_url: str) -> Path:
        ...
        filename = safe_filename(source_url.split("/")[-1] or "vigilance_plan.pdf")
