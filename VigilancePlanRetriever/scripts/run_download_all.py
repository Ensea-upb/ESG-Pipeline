...
from src.vigilance_plan_retriever.models import Company
from src.vigilance_plan_retriever.retriever import VigilancePlanRetriever
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Télécharger tous les candidats plans de vigilance / devoir de vigilance "
            "dont le score dépasse un seuil."
        )
    )

    parser.add_argument("--company-name", required=True, help="Nom de l'entreprise.")
    parser.add_argument("--fiscal-year", required=True, type=int, help="Année cible.")
    ...
    retriever = VigilancePlanRetriever(root_dir=Path(args.root_dir))
    ...
    print("\n=== Vigilance Plan Retriever Result ===")
    ...
    output_path = (
        output_dir
        / f"download_all_vigilance_{safe_company_name}_{args.fiscal_year}.json"
    )
