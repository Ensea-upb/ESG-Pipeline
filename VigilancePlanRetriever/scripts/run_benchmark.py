...
def print_summary(records: list[dict]) -> None:
    print("\n" + "=" * 100)
    print("SYNTHÈSE DU BENCHMARK VIGILANCE PLAN")
    print("=" * 100)
...
def save_benchmark_report(records: list[dict], root_dir: Path, fiscal_year: int) -> Path:
    ...
    output_path = registry_dir / f"benchmark_vigilance_plan_{fiscal_year}_{timestamp}.json"

    output_path.write_text(
        json.dumps(
            {
                "benchmark_name": "vigilance_plan_retriever_benchmark",
                "fiscal_year": fiscal_year,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark du moteur VigilancePlanRetriever."
    )
    ...
    retriever = VigilancePlanRetriever(root_dir=root_dir)
