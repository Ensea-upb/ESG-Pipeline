...
def print_summary(records: list[dict]) -> None:
    print("\n" + "=" * 100)
    print("SYNTHÈSE DU BENCHMARK GOVERNANCE REPORT")
    print("=" * 100)
...
def save_benchmark_report(records: list[dict], root_dir: Path, fiscal_year: int) -> Path:
    ...
    output_path = registry_dir / f"benchmark_governance_report_{fiscal_year}_{timestamp}.json"

    output_path.write_text(
        json.dumps(
            {
                "benchmark_name": "governance_report_retriever_benchmark",
                "fiscal_year": fiscal_year,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark du moteur GovernanceReportRetriever."
    )
    ...
    retriever = GovernanceReportRetriever(root_dir=root_dir)
