...
from .models import GovernanceReportRequest, SearchCandidate, ScoredCandidate
from .utils import (
...
# ============================================================
# 1. Mots-clés centraux : rapports de gouvernance
# ============================================================

STRONG_GOVERNANCE_KEYWORDS = [
    # Anglais
...
GOVERNANCE_DISCLOSURE_KEYWORDS = [
    # Anglais — référentiels & pratiques
    "afep medef",
    "afep-medef",
...
# Rapports annuels / URD : utiles si la gouvernance y est intégrée.
...
# Convocations d'assemblée : à distinguer des rapports de gouvernance.
# Une convocation n'est PAS un rapport de gouvernance.
...
AUTO_DOWNLOAD_ALLOWED_CLASSES = {
    "standalone_governance_report",
    "governance_disclosure",
    "annual_report_with_governance_section",
}
...
class CandidateFeatures:
    ...
    strong_governance_keywords: list[str]
    governance_disclosure_keywords: list[str]
    weak_governance_keywords: list[str]
...
def find_keywords(text: str, keywords: list[str]) -> list[str]:
    ...
    for keyword in keywords:
        ...
        # Use word-boundary matching for short tokens to avoid URL fragment false positives.
...
class GovernanceReportScorer:
    """
    Scorer généraliste pour les rapports de gouvernance d'entreprise.

    Distinctions clés :
    - Un rapport de gouvernance décrit la structure, les principes et les pratiques
      de gouvernance du conseil d'administration.
    - Il ne doit pas être confondu avec un avis de convocation d'assemblée générale
      (AGM notice), qui est une convocation formelle aux actionnaires.
    ...
    def score_candidate(
        ...
        if features.strong_governance_keywords:
            score += 35
            positive_signals.append(
                f"mots-clés forts gouvernance : {features.strong_governance_keywords}"
            )
        elif features.governance_disclosure_keywords:
            score += 35
            positive_signals.append(
                f"mots-clés divulgation gouvernance : {features.governance_disclosure_keywords}"
            )
        elif features.weak_governance_keywords:
            score += 18
            positive_signals.append(
                f"mots-clés faibles gouvernance : {features.weak_governance_keywords}"
            )
        else:
            score -= 25
            negative_signals.append("aucun signal gouvernance fort")
        ...
        if features.strong_filename_keywords and features.is_pdf:
            score += 15
            positive_signals.append(
                f"nom de fichier compatible gouvernance : {features.strong_filename_keywords}"
            )

        if features.annual_report_context_keywords and (
            features.strong_governance_keywords
            or features.governance_disclosure_keywords
            or features.weak_governance_keywords
        ):
            score += 8
            positive_signals.append(
                f"rapport annuel/URD contenant signaux gouvernance : {features.annual_report_context_keywords}"
            )
        ...
        if features.financial_only_keywords and not (
            ...
            score -= 45
            negative_signals.append(
                f"document financier sans signal gouvernance fort : {features.financial_only_keywords}"
            )
    ...
    def extract_features(
        ...
        return CandidateFeatures(
            ...
            strong_governance_keywords=find_keywords(
                combined_text,
                STRONG_GOVERNANCE_KEYWORDS,
            ),
            governance_disclosure_keywords=find_keywords(
                combined_text,
                GOVERNANCE_DISCLOSURE_KEYWORDS,
            ),
            weak_governance_keywords=find_keywords(
                combined_text,
                WEAK_GOVERNANCE_KEYWORDS,
            ),
            ...
            agm_convocation_keywords=find_keywords(
                combined_text,
                AGM_CONVOCATION_KEYWORDS,
            ),
    ...
    def classify_candidate(self, features: CandidateFeatures) -> str:
        ...
        Distinction clé entre :
        - un rapport de gouvernance (standalone_governance_report)
        - un avis de convocation d'AG (agm_convocation_document) → à rejeter
        """
        ...
        # Un avis de convocation n'est PAS un rapport de gouvernance.
        if features.agm_convocation_keywords and not features.strong_governance_keywords:
        ...
        if features.governance_disclosure_keywords:
            return "governance_disclosure"

        if features.strong_governance_keywords:
            return "standalone_governance_report"

        if (
            features.annual_report_context_keywords
            and features.weak_governance_keywords
        ):
            return "annual_report_with_governance_section"
