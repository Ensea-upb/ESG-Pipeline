...
from .scorer import GovernanceReportScorer
from .search import GovernanceReportSearch
from .storage import GovernanceReportStorage
from .utils import extract_years, safe_filename, slugify


class GovernanceReportRetriever:
    """
    Moteur central d'ingestion des rapports de gouvernance d'entreprise.

    Rôle :
    - rechercher les candidats (Corporate Governance Report, Governance Statement,
      Governance Charter, Governance Framework, Code de gouvernance) ;
    - scorer les candidats, en distinguant les vrais rapports de gouvernance
      des avis de convocation d'assemblée générale ;
    ...
    - stocker les PDF dans data/dossier_ingestion_0/governance_reports/ ;
    - créer les manifests.
    ...
    def __init__(
        ...
        self.search = GovernanceReportSearch()

        self.scorer = GovernanceReportScorer(
            auto_download_threshold=auto_download_threshold,
            human_review_threshold=human_review_threshold,
        )

        self.downloader = PDFDownloader()
        self.storage = GovernanceReportStorage(root_dir=self.root_dir)
    ...
    def download_high_score_candidates(
        ...
        """
        Télécharge tous les candidats rapports de gouvernance
        ...
        Pipeline :
            ...
            téléchargement
                ↓
            stockage dans governance_reports/
        ...
        for source_search_rank, candidate in enumerate(selected_candidates, start=1):
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in resolved_scored_candidates:
                    ...
                    manifest = self.storage.store_candidate_governance_report(
        ...
        return DownloadResult(
            status="completed",
            message="Ingestion des candidats rapports de gouvernance terminée.",
    ...
    def _build_temp_pdf_path(
        ...
        filename = safe_filename(
            source_url.split("/")[-1] or "governance_report.pdf"
        )
