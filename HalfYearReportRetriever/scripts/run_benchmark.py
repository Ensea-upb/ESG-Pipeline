...
from src.half_year_report_retriever.models import Company
from src.half_year_report_retriever.retriever import HalfYearReportRetriever
...
def result_to_record(company: Company, fiscal_year: int, result: Any) -> dict:
    return {
        "company_name": company.name, "official_domain": company.official_domain,
...
def print_record(record: dict) -> None:
    ...
    if record["failed"]:
        ...
        for item in record["failed"]:
            print(f"  - [{item['rank']}] score={item['score']} | {item['title']} | {item['download_status']}")
    if record["skipped"]:
        print("\nIgnorés:")
        for item in record["skipped"]:
            print(f"  - [{item['rank']}] score={item['score']} | {item['title']} | {item['decision']}")


def print_summary(records: list[dict]) -> None:
    print("\n" + "=" * 100)
    print("SYNTHÈSE DU BENCHMARK HALF YEAR REPORT")
    print("=" * 100)
    header = f"{'Entreprise':<25} {'Status':<15} {'Downloaded':<12} {'Failed':<8} {'Skipped':<8}"
    print(header)
    print("-" * len(header))
    for record in records:
        print(f"{record['company_name'][:24]:<25} {str(record['status'])[:14]:<15} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark du moteur HalfYearReportRetriever.")
    ...
    retriever = HalfYearReportRetriever(root_dir=root_dir)
    records: list[dict] = []
    ...
    for company_config in DEFAULT_COMPANIES:
        ...
        try:
            result = retriever.download_high_score_candidates(
                company=company, fiscal_year=args.fiscal_year, min_score=args.min_score,
            )
    ...
    output_path = registry_dir / f"benchmark_half_year_report_{args.fiscal_year}_{timestamp}.json"
    output_path.write_text(json.dumps({
        "benchmark_name": "half_year_report_retriever_benchmark",
        "fiscal_year": args.fiscal_year,
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "total_companies": len(records), "records": records,
