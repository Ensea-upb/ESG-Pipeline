...
from .models import Company, DownloadResult, HalfYearReportRequest, ScoredCandidate
from .scorer import HalfYearReportScorer
from .search import HalfYearReportSearch
from .storage import HalfYearReportStorage
from .utils import extract_years, safe_filename, slugify


class HalfYearReportRetriever:
    """
    Moteur central d'ingestion des rapports financiers semestriels (H1).

    Cible :
    - Rapport semestriel (rapport financier semestriel, comptes semestriels) ;
    - Half Year Report / Semi-Annual Report ;
    - Interim Report (H1) ;
    - Résultats du premier semestre.

    Stocke dans : data/dossier_ingestion_0/half_year_reports/
    """
    ...
    def __init__(
        ...
        self.search = HalfYearReportSearch()
        self.scorer = HalfYearReportScorer(
            auto_download_threshold=auto_download_threshold,
            human_review_threshold=human_review_threshold,
        )
        self.downloader = PDFDownloader()
        self.storage = HalfYearReportStorage(root_dir=self.root_dir)
    ...
    def download_high_score_candidates(
        ...
        """
        Télécharge tous les candidats rapports semestriels dont le score
        est supérieur ou égal à min_score.

        Règle temporelle : accepte fiscal_year et fiscal_year - 1.
        """
        request = HalfYearReportRequest(company=company, fiscal_year=fiscal_year)
        ...
        for source_search_rank, candidate in enumerate(selected_candidates, start=1):
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in self.scorer.rank_candidates(
                    ...
                    manifest = self.storage.store_candidate_half_year_report(
        ...
        return DownloadResult(
            status="completed",
            message="Ingestion des candidats rapports semestriels terminée.",
    ...
    def _build_temp_pdf_path(self, company: Company, fiscal_year: int, source_url: str) -> Path:
        ...
        filename = safe_filename(source_url.split("/")[-1] or "half_year_report.pdf")
