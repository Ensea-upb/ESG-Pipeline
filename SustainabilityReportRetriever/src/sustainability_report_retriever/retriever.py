...
from .scorer import SustainabilityReportScorer
from .search import SustainabilityReportSearch
from .storage import SustainabilityReportStorage
from .utils import extract_years, safe_filename, slugify


class SustainabilityReportRetriever:
    """
    Moteur central d'ingestion des rapports ESG / Sustainability / RSE.
    ...
    - stocker les PDF dans dossier_ingestion_0 ;
    - créer les manifests.
    ...
    def __init__(
        ...
        self.search = SustainabilityReportSearch()

        self.scorer = SustainabilityReportScorer(
            auto_download_threshold=auto_download_threshold,
            human_review_threshold=human_review_threshold,
        )

        self.downloader = PDFDownloader()
        self.storage = SustainabilityReportStorage(root_dir=self.root_dir)
    ...
    def download_high_score_candidates(
        ...
        """
        Télécharge tous les candidats Sustainability / ESG / RSE
        ...
        for source_search_rank, candidate in enumerate(selected_candidates, start=1):
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in resolved_scored_candidates:
                    ...
                    manifest = self.storage.store_candidate_sustainability_report(
            ...
            # Filtre temporel appliqué aussi aux candidats directs.
            # Cela évite de télécharger des archives anciennes qui ont
            # été proposées directement par Tavily.
            if not self._is_candidate_year_relevant(
                candidate=candidate,
                fiscal_year=fiscal_year,
            ):
        ...
        return DownloadResult(
            status="completed",
            message="Ingestion des candidats Sustainability / ESG / RSE terminée.",
    ...
    def _build_temp_pdf_path(
        ...
        filename = safe_filename(
            source_url.split("/")[-1] or "sustainability_report.pdf"
        )
