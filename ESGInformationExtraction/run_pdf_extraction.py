...
# ── Layer-2 extraction modules (optional — graceful fallback if missing) ──────
_EXTRACTION_MODULES_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(_EXTRACTION_MODULES_DIR))

def _try_import_metric_extractor():
    try:
        from extraction.metric_candidate_extractor import extract_metric_candidates
        return extract_metric_candidates
    except Exception:
        return None

def _try_import_gliner():
    try:
        from extraction.gliner_extractor import extract_with_gliner, merge_candidates
        return extract_with_gliner, merge_candidates
    except Exception:
        return None, None

def _try_import_unit_normalizer():
    try:
        from extraction.unit_normalizer import normalize_unit
        return normalize_unit
    except Exception:
        return None

def _try_import_docling_parser():
    try:
        from parsing.docling_parser import parse_pdf_docling
...
_extract_metric_candidates = _try_import_metric_extractor()
_extract_with_gliner, _merge_candidates = _try_import_gliner()
_normalize_unit = _try_import_unit_normalizer()
_parse_pdf_docling = _try_import_docling_parser()
_pytesseract = _try_import_pytesseract()
...
SECTION_CANDIDATE_SCORE_THRESHOLD = 0.45
...
class ExtractionOptions:
    ...
    max_pages: int | None
    overwrite: bool
    # Optional metadata fields (all default None = not provided)
...
SECTION_TYPE_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("health_safety", (r"\bhealth and safety\b", r"\bsante securite\b", r"\bsafety\b", r"\bsecurite au travail\b", r"\boccupational health\b", r"\bsante des salaries\b")),
    ...
    ("human_rights", (r"\bhuman rights\b", r"\bdroits humains\b", r"\bdroits de l homme\b", r"\bdroits fondamentaux\b", r"\btravail force\b", r"\bforced labour\b")),
    ("remuneration", (r"\bremuneration\b", r"\bcompensation\b", r"\bremuneration report\b", r"\bpolitique salariale\b", r"\bpay equity\b", r"\begalite salariale\b")),
    ("biodiversity", (r"\bbiodiversity\b", r"\bbiodiversite\b", r"\bnature[- ]positive\b", r"\becosystem\b", r"\becosysteme\b", r"\bbiodiversity footprint\b")),
    ("governance", (r"\bgovernance\b", r"\bgouvernance\b", r"\bboard of directors\b", r"\bconseil d administration\b", r"\bconseil d administration\b", r"\badministrateurs\b")),
    ("vigilance", (r"\bvigilance plan\b", r"\bplan de vigilance\b", r"\bduty of vigilance\b", r"\bdevoir de vigilance\b")),
    ("assurance", (r"\bassurance\b", r"\bverification\b", r"\blimited assurance\b", r"\baudit independant\b", r"\btierce partie\b")),
    ("emissions", (r"\bemissions\b", r"\bghg\b", r"\bscope 1\b", r"\bscope 2\b", r"\bscope 3\b", r"\bcarbone\b", r"\bco2\b", r"\bgreenhouse gas\b")),
    ("taxonomy", (r"\btaxonomy\b", r"\btaxonomie\b", r"\bcsrd\b", r"\bdpef\b", r"\bsustainable finance\b", r"\bfinance durable\b")),
    ("climate", (r"\bclimate\b", r"\bclimat\b", r"\btcfd\b", r"\btransition plan\b", r"\bnet[- ]zero\b", r"\bzero emission\b", r"\bneutralite carbone\b", r"\bcarbon neutral\b", r"\bclimat changement\b", r"\bchangement climatique\b", r"\btransition energetique\b")),
    ("energy", (r"\benergy\b", r"\benergie\b", r"\bconsommation energetique\b", r"\brenewable energy\b", r"\benergie renouvelable\b", r"\belectricite\b")),
    ("water", (r"\bwater\b", r"\bwater withdrawal\b", r"\beau\b", r"\bgestion de l eau\b", r"\bwater management\b", r"\bconsommation d eau\b", r"\bhydrique\b")),
    ("waste", (r"\bwaste\b", r"\bdechets\b", r"\beconomie circulaire\b", r"\bcircular economy\b", r"\brecyclage\b", r"\brecycling\b", r"\bvalorisation\b")),
    ("diversity", (r"\bdiversity\b", r"\binclusion\b", r"\bdiversite\b", r"\bdei\b", r"\bequite\b", r"\bparite\b", r"\bgender equality\b", r"\begalite femmes hommes\b")),
    ("workforce", (r"\bworkforce\b", r"\bemployees\b", r"\beffectifs\b", r"\bsalaries\b", r"\bcollaborateurs\b", r"\bformation\b", r"\btalent\b", r"\bengagement employes\b")),
    ("ethics", (r"\bethics\b", r"\bethical\b", r"\bethique\b", r"\bcode of conduct\b", r"\bcode de conduite\b", r"\bcompliance\b", r"\bconformite\b")),
    ("supply_chain", (r"\bsupply chain\b", r"\bchaine d approvisionnement\b", r"\bachats responsables\b", r"\bresponsible sourcing\b", r"\bfournisseurs\b", r"\bsuppliers\b")),
    ("community", (r"\bcommunity\b", r"\bcommunautes\b", r"\bterritoires\b", r"\bimpact local\b", r"\bengagement communautaire\b", r"\bfondation\b")),
    ("reporting", (r"\breporting\b", r"\bindice gri\b", r"\bgri index\b", r"\bsasb\b", r"\bun sdg\b", r"\bods\b", r"\bindicateurs esg\b", r"\besg indicators\b")),
    ("environment", (r"\benvironment\b", r"\benvironnement\b", r"\bpilier environnemental\b", r"\benvironmental performance\b")),
    ("social", (r"\bsocial\b", r"\bpilier social\b", r"\bresponsabilite sociale\b", r"\bsocial performance\b")),
    ("general", (r"\boverview\b", r"\bintroduction\b", r"\bprofile\b", r"\bgeneral\b", r"\bfinancial highlights\b", r"\bhighlights\b", r"\bperformance\b", r"\bchiffres cles\b", r"\bkey figures\b")),
)
...
def build_audit_findings(
    ...
    # Files written after this audit step (quality_report, extraction_summary)
    # and the audit files themselves must be excluded from the missing-file check
    # to avoid false-positive critical findings.
    self_audit_files = {
        "consistency_report.json", "audit_findings.jsonl", "document_audit_report.md",
        "quality_report.jsonl", "extraction_summary.json",
    }
...
def _build_pages_for_extraction(text_blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Reconstruct page-level text from individual text blocks for layer-2 extractors."""
    pages: dict[int, dict[str, Any]] = {}
    for block in text_blocks:
        p = int(block.get("page_number") or 0)
        text = str(block.get("text") or "").strip()
        if not text:
            continue
        if p not in pages:
            pages[p] = {"page_number": p, "text": ""}
        pages[p]["text"] += text + "\n"
    return sorted(pages.values(), key=lambda x: x["page_number"])


def _run_docling_in_subprocess(pdf_path: "Path") -> "dict[int, str]":
    """Run Docling table extraction in a child process.

    Docling's C++ preprocessing can raise std::bad_alloc on large documents,
    killing the entire process. Running it in a subprocess keeps the main
    pipeline alive regardless of what happens inside Docling.

    Returns a {page_number: table_markdown_text} dict, empty on any failure.
    """
    import subprocess
    script = (
        "import json, sys\n"
        f"sys.path.insert(0, r'{str(_EXTRACTION_MODULES_DIR)}')\n"
        f"from parsing.docling_parser import parse_pdf_docling\n"
        f"from pathlib import Path\n"
        "pages = parse_pdf_docling(Path(r'" + str(pdf_path) + "'))\n"
        "result = {}\n"
        "for p in pages:\n"
        "    pno = int(p.get('page_number') or 0)\n"
        "    tables = p.get('tables_structured') or []\n"
        "    md = '\\n'.join(t.get('markdown') or '' for t in tables if t.get('markdown'))\n"
        "    if md.strip() and p.get('parser') == 'docling':\n"
        "        result[str(pno)] = md\n"
    ...
    try:
        ...
        log.debug("Docling subprocess failed: %s", exc)
    return {}
...
def _run_metric_extraction(
    ...
    """Run regex + GLiNER metric extraction and unit normalization.

    Returns (candidates, statistics). Always returns safely even if layer-2
    modules are unavailable — the pipeline never crashes here.
    """
    if _extract_metric_candidates is None:
        return [], {"status": "unavailable", "reason": "metric_candidate_extractor not importable"}

    pages = _build_pages_for_extraction(text_blocks)
    if not pages:
        return [], {"status": "no_pages", "candidates_count": 0}

    # Docling enrichment: subprocess-isolated to avoid C++ OOM killing the pipeline.
    # Skipped automatically for documents > DOCLING_MAX_PAGES pages.
    DOCLING_MAX_PAGES = 80
    docling_status = "unavailable"
    page_count = len(pages)
    if _parse_pdf_docling is not None and pdf_path is not None:
        if page_count > DOCLING_MAX_PAGES:
            docling_status = f"skipped:doc_too_large({page_count}p)"
        else:
            docling_table_text = _run_docling_in_subprocess(pdf_path)
            if docling_table_text:
                for page in pages:
                    extra = docling_table_text.get(page["page_number"], "")
                    if extra:
                        page["text"] = page["text"] + "\n" + extra
                docling_status = f"enriched:{len(docling_table_text)}_pages"
            else:
                docling_status = "subprocess_failed_or_no_tables"

    # Parse fiscal_year to int for temporal discrimination
    fy_int: int | None = None
    if fiscal_year:
        try:
            fy_int = int(str(fiscal_year)[:4])
        except (ValueError, TypeError):
            pass

    # Step 1: regex + keyword catalog
    try:
        regex_candidates = _extract_metric_candidates(pages, fiscal_year=fy_int) or []
    except Exception as exc:
        log.warning("metric_candidate_extractor failed: %s", exc)
        regex_candidates = []

    # Step 2: GLiNER NER (optional)
    gliner_candidates: list[dict[str, Any]] = []
    gliner_status = "unavailable"
    if _extract_with_gliner is not None:
        try:
            gliner_candidates = _extract_with_gliner(pages) or []
            gliner_status = "success" if gliner_candidates else "no_results"
        except Exception as exc:
            log.warning("gliner_extractor failed: %s", exc)
            gliner_status = f"error:{exc}"

    # Step 3: merge
    if _merge_candidates is not None:
        candidates = _merge_candidates(regex_candidates, gliner_candidates)
    else:
        candidates = regex_candidates + gliner_candidates

    # Step 4: unit normalization (raw_value, raw_unit, metric_id)
    if _normalize_unit is not None:
        for cand in candidates:
            raw_unit = str(cand.get("raw_unit") or "")
            raw_value = str(cand.get("raw_value") or "")
            if raw_unit and raw_value:
                try:
                    result = _normalize_unit(raw_value, raw_unit, cand.get("metric_id"))
                    cand["normalized_unit"] = result.get("canonical_unit") or ""
                    cand["normalized_value"] = result.get("normalized_value")
                    cand["normalization_status"] = result.get("normalization_status")
                except Exception:
                    pass

    # Add document_id to each candidate
    for cand in candidates:
        cand.setdefault("document_id", document_id)
    ...
    stats = {
        ...
        "gliner_candidates_count": len(gliner_candidates),
        "gliner_status": gliner_status,
        "merged_candidates_count": len(candidates),
        "candidates_with_value": sum(1 for c in candidates if c.get("raw_value")),
        "candidates_with_unit": sum(1 for c in candidates if c.get("raw_unit")),
        "candidates_with_normalized_unit": sum(1 for c in candidates if c.get("normalized_unit")),
        "unique_metric_ids": len({c.get("metric_id") or c.get("metric_label") for c in candidates}),
        "candidates_current_year": sum(1 for c in candidates if c.get("is_current_year") is True),
        "candidates_historical": sum(1 for c in candidates if c.get("is_current_year") is False),
        "candidates_year_unknown": sum(1 for c in candidates if c.get("is_current_year") is None),
        "docling_status": docling_status,
        "ocr_available": _pytesseract is not None,
    }
...
def run_extraction(options: ExtractionOptions) -> tuple[int, dict[str, Any]]:
    ...
    # Build optional metadata dict from CLI args (only non-None values)
    _cli_metadata: dict[str, Any] = {}
    if options.company is not None:
        _cli_metadata["company"] = options.company
    if options.company_name is not None:
        _cli_metadata["company_name"] = options.company_name
    if options.company_slug is not None:
        _cli_metadata["company_slug"] = options.company_slug
    if options.fiscal_year is not None:
        _cli_metadata["fiscal_year"] = options.fiscal_year
    if options.official_doc_type is not None:
        _cli_metadata["official_doc_type"] = options.official_doc_type
    if options.document_family is not None:
        _cli_metadata["document_family"] = options.document_family
    if options.final_path is not None:
        _cli_metadata["final_path"] = options.final_path
    if options.source_index_path is not None:
        _cli_metadata["source_index_path"] = options.source_index_path
    if options.corpus_run_id is not None:
        _cli_metadata["corpus_run_id"] = options.corpus_run_id
    ...
    try:
        ...
        # ── v0.9: metric extraction (layer-2 — regex catalog + GLiNER NER + Docling) ──
        metric_candidates, metric_statistics = _run_metric_extraction(
            options.document_id, text_blocks,
            fiscal_year=options.fiscal_year,
            pdf_path=options.pdf_path,
        )
        if _cli_metadata:
            metric_candidates = [{**c, **_cli_metadata} for c in metric_candidates]
        write_jsonl(output_dir / "metric_candidates.jsonl", metric_candidates)
        write_json(output_dir / "metric_statistics.json", metric_statistics)
        ...
        # Inject CLI metadata additively into each evidence record (at end of record)
        if _cli_metadata:
            evidence_records = [{**rec, **_cli_metadata} for rec in evidence_records]
        ...
        # Inject CLI metadata additively into document_inventory (at end)
        if _cli_metadata:
            document_inventory = {**document_inventory, **_cli_metadata}
        write_json(output_dir / "document_inventory.json", document_inventory)
        # Inject CLI metadata additively into each multimodal evidence record (at end of record)
        if _cli_metadata:
            multimodal_records = [{**rec, **_cli_metadata} for rec in multimodal_records]
        ...
        try:
            ...
            write_jsonl(output_dir / "section_index.jsonl", sections)
            write_json(output_dir / "section_statistics.json", section_statistics)
            # Inject CLI metadata additively into evidence records in error path too
            if _cli_metadata:
                evidence_records = [{**rec, **_cli_metadata} for rec in evidence_records]
            write_jsonl(output_dir / "evidence_store.jsonl", evidence_records)
            write_json(output_dir / "evidence_statistics.json", evidence_statistics)
    ...
    summary = {
        ...
        # v0.9 metric extraction stats
        "metric_candidates_count": len(metric_candidates),
        "metric_candidates_with_value": int(metric_statistics.get("candidates_with_value", 0) or 0),
        "metric_regex_candidates_count": int(metric_statistics.get("regex_candidates_count", 0) or 0),
        "metric_gliner_candidates_count": int(metric_statistics.get("gliner_candidates_count", 0) or 0),
        "metric_gliner_status": metric_statistics.get("gliner_status", "not_run"),
        "metric_unique_metric_ids": int(metric_statistics.get("unique_metric_ids", 0) or 0),
...
def parse_args(argv: list[str] | None = None) -> ExtractionOptions:
    ...
    # Optional metadata arguments — all default None (backward compatible)
    parser.add_argument("--company", default=None, help="Reporting company name (e.g. TotalEnergies)")
    parser.add_argument("--company-name", default=None, help="Full legal company name")
    parser.add_argument("--company-slug", default=None, help="URL/filesystem-safe company slug")
    parser.add_argument("--fiscal-year", default=None, help="Fiscal/reporting year (e.g. 2024)")
    parser.add_argument("--official-doc-type", default=None, help="Official document type (e.g. URD)")
    parser.add_argument("--document-family", default=None, help="Document family classification")
    parser.add_argument("--final-path", default=None, help="Canonical path within ESGFinalCorpus")
    parser.add_argument("--source-index-path", default=None, help="Path to source index file")
    parser.add_argument("--corpus-run-id", default=None, help="Corpus run identifier")
    ...
    return ExtractionOptions(
        ...
        company_slug=args.company_slug or None,
        fiscal_year=args.fiscal_year or None,
        official_doc_type=args.official_doc_type or None,
        document_family=args.document_family or None,
        final_path=args.final_path or None,
        source_index_path=args.source_index_path or None,
        corpus_run_id=args.corpus_run_id or None,
    )
