...
from .models import Company, DownloadResult, RemunerationReportRequest, ScoredCandidate
from .scorer import RemunerationReportScorer
from .search import RemunerationReportSearch
from .storage import RemunerationReportStorage
from .utils import extract_years, safe_filename, slugify


class RemunerationReportRetriever:
    """
    Moteur central d'ingestion des rapports de rémunération des dirigeants.

    Cible : Remuneration Report, Directors' Remuneration Report, Executive
    Compensation Report, Pay Report, Politique de rémunération, Rapport sur
    les rémunérations des mandataires sociaux.

    Stocke dans : data/dossier_ingestion_0/remuneration_reports/
    """
    ...
    def __init__(
        ...
        self.search = RemunerationReportSearch()
        self.scorer = RemunerationReportScorer(
            auto_download_threshold=auto_download_threshold,
            human_review_threshold=human_review_threshold,
        )
        self.downloader = PDFDownloader()
        self.storage = RemunerationReportStorage(root_dir=self.root_dir)
    ...
    def download_high_score_candidates(
        ...
        """
        Télécharge tous les candidats rapports de rémunération dont le score
        est supérieur ou égal à min_score.

        Règle temporelle : accepte fiscal_year et fiscal_year - 1.
        """
        request = RemunerationReportRequest(company=company, fiscal_year=fiscal_year)
        ...
        for source_search_rank, candidate in enumerate(selected_candidates, start=1):
            ...
            if candidate.decision == "pdf_resolution_required":
                ...
                for resolved_candidate in self.scorer.rank_candidates(
                    ...
                    manifest = self.storage.store_candidate_remuneration_report(
        ...
        if resolved_year_irrelevant_count > 0:
            skipped.append({"rank": "?", "title": "PDFs extraits HTML hors période cible",
                "url": None, "score": None, "decision": "reject",
        ...
        if direct_year_irrelevant_count > 0:
            skipped.append({"rank": "?", "title": "Candidats directs hors période cible",
                "url": None, "score": None, "decision": "reject",
                "reason": f"{direct_year_irrelevant_count} candidat(s) hors période. Acceptés : {fiscal_year} et {fiscal_year - 1}."})

        return DownloadResult(
            status="completed",
            message="Ingestion des candidats rapports de rémunération terminée.",
    ...
    @staticmethod
    def _is_candidate_year_relevant(candidate: ScoredCandidate, fiscal_year: int) -> bool:
        years = extract_years(f"{candidate.title} {candidate.url}")
        if not years:
            return True
        return bool(years & {fiscal_year, fiscal_year - 1})

    def _build_temp_pdf_path(self, company: Company, fiscal_year: int, source_url: str) -> Path:
        self.tmp_dir.mkdir(parents=True, exist_ok=True)
        company_slug = slugify(company.name)
        filename = safe_filename(source_url.split("/")[-1] or "remuneration_report.pdf")
