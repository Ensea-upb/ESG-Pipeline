...
def print_summary(records, label):
    print(f"\n{'='*90}\nSYNTHESE DU BENCHMARK ASSURANCE REPORT\n{'='*90}")
...
def main():
    ...
    retriever = AssuranceReportRetriever(root_dir=root_dir)
    records = []
    ...
    print_summary(records, "ASSURANCE REPORT")
    registry_dir = root_dir / "registry"; registry_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    safe_label = "ASSURANCE REPORT".lower().replace(" ", "_")
    output_path = registry_dir / f"benchmark_{safe_label}_{args.fiscal_year}_{timestamp}.json"
    output_path.write_text(json.dumps({"benchmark_name": f"ASSURANCE REPORT_retriever_benchmark",
