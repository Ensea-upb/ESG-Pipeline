...
from .models import Company, AssuranceReportRequest, SearchCandidate, ScoredCandidate

STRONG_ASSURANCE_KEYWORDS = [
    "third party assurance report", "third-party assurance report",
    "independent verification statement", "independent assurance statement",
    "limited assurance report", "reasonable assurance report",
    "assurance report sustainability", "assurance report esg",
    "rapport assurance esg", "rapport vérification durabilité",
    "rapport assurance développement durable",
    "commissaire aux comptes informations durabilité",
    "isae 3000 assurance", "isae 3410 assurance",
    "aa1000 assurance", "independent assurance engagement",
    "sustainability assurance report", "esg verification report",
    "ghg verification statement", "ghg assurance report",
    "csrd assurance report", "csrd assurance tiers",
    "rapport du commissaire aux comptes sur les informations de durabilite",
]

SECONDARY_ASSURANCE_KEYWORDS = [
    "assurance", "verification", "vérification", "audit ESG",
    "limited assurance", "reasonable assurance",
    "deloitte assurance", "pwc assurance", "kpmg assurance", "ey assurance",
    "bureau veritas", "rina", "dnv", "kpmg sustainability",
    "pwc sustainability", "deloitte sustainability",
    "independent reviewer", "tiers vérificateur",
    "assurance provider", "assurance scope",
]

WEAK_ASSURANCE_KEYWORDS = [
    "verified", "certified", "audited", "vérifié", "certifié", "audité",
    "independent review", "external review",
]

STRONG_FILENAME_KEYWORDS = [
    "assurance-report", "assurancereport", "assurance_report",
    "verification-statement", "verificationstatement",
    "assurance-statement", "esg-assurance", "ghg-verification",
    "independent-assurance", "tiers-assurance", "rapport-assurance",
]
...
class AssuranceReportScorer:
    ...
    def rank_candidates(self, request: AssuranceReportRequest, candidates: list[SearchCandidate]) -> list[ScoredCandidate]:
        scored = [self._score(request=request, candidate=c) for c in candidates]
        scored.sort(key=lambda c: c.score, reverse=True)
        return scored

    def _score(self, request: AssuranceReportRequest, candidate: SearchCandidate) -> ScoredCandidate:
        ...
        strong_kws = find_keywords(text_norm, STRONG_ASSURANCE_KEYWORDS)
        if strong_kws:
            score += 45
            positive.append(f"mots-clés forts assurance : {strong_kws[:3]}")

        secondary_kws = find_keywords(text_norm, SECONDARY_ASSURANCE_KEYWORDS)
        if secondary_kws:
            score += 20
            positive.append(f"mots-clés secondaires : {secondary_kws[:3]}")

        weak_kws = find_keywords(text_norm, WEAK_ASSURANCE_KEYWORDS)
        ...
        if normalize_text(company.name) in text_norm:
            score += 15
        ...
        if fn_kws:
            score += 15
            positive.append(f"nom de fichier assurance : {fn_kws[:2]}")
        ...
        if not strong_kws and not secondary_kws:
            ...
            document_class = "assurance_report"
